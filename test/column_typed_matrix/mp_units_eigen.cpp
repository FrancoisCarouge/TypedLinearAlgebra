...
//! @test Verifies the column_typed_matrix concept over mp-units quantity
//! indexes, Eigen backend.

using column =
    matrix<double, std::tuple<position, velocity>, std::tuple<std::identity>>;
static_assert(column_typed_matrix<column>);
static_assert(column_typed_matrix<const column &>);

using row =
    matrix<double, std::tuple<std::identity>, std::tuple<position, velocity>>;
static_assert(not column_typed_matrix<row>);

using square = matrix<double, std::tuple<position, velocity>,
                      std::tuple<position, velocity>>;
static_assert(not column_typed_matrix<square>);
} // namespace
} // namespace fcarouge::test
