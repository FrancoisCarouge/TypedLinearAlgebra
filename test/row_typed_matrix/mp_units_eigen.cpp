...
//! @test Verifies the row_typed_matrix concept over mp-units quantity indexes,
//! Eigen backend.

using row =
    matrix<double, std::tuple<std::identity>, std::tuple<position, velocity>>;
static_assert(row_typed_matrix<row>);
static_assert(row_typed_matrix<const row &>);

using column =
    matrix<double, std::tuple<position, velocity>, std::tuple<std::identity>>;
static_assert(not row_typed_matrix<column>);

using square = matrix<double, std::tuple<position, velocity>,
                      std::tuple<position, velocity>>;
static_assert(not row_typed_matrix<square>);
} // namespace
} // namespace fcarouge::test
