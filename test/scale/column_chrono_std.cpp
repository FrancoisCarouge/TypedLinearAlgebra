...
namespace {
//! @test Verifies the scale algorithm for a column vector of std::chrono
...
[[maybe_unused]] const auto test{[] -> int {
  ...
  column_vector<representation, seconds, minutes> x{span};
  ...
  assert(seconds{3.} == x.at<0>());
  assert(minutes{6.} == x.at<1>());
