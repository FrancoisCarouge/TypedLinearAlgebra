...
//! @test Verifies the formatter properties for the nholthaus/units and mdspan
//! composition.
...
//! @test Verifies the format algorithm for nholthaus/units quantity typed
//! matrices with the mdspan backend: the rank-0 singleton, the row vector, the
//! column vector, and the general rectangular overloads.
[[maybe_unused]] const auto test{[] -> int {
  ...
  e.at<1, 0>(3. * m2);
  e.at<1, 1>(4. * m2);
  assert(std::format("{}", e) == "[[1 m2, 2 m2], [3 m2, 4 m2]]");
