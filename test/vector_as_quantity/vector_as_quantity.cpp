...
inline constexpr struct velocity3d
    : mp_units::quantity_spec<mp_units::isq::velocity> {
} velocity3d;

inline constexpr struct forward_velocity
    : mp_units::quantity_spec<mp_units::isq::velocity, is_kind> {
} forward_velocity;
...
template <>
struct mp_units::vector_components<velocity3d>
    : mp_units::vector_axes<forward_velocity, lateral_velocity,
                            vertical_velocity> {};

// Okay, let's hack it: my rep is already type safe, thank you.
...
[[maybe_unused]] const auto test{[] {
  ...
  // DO BETTER: NAME THE AXIS HERE AND AVOID THE ADDITIONAL LAYER.
  using vector3v = typed_column_vector<vector3d, quantity<isq::velocity[m / s]>,
                                       quantity<isq::velocity[m / s]>,
                                       quantity<isq::velocity[m / s]>>;

  // Conversion from a vector as a quantity.
  // vector3v velocities{velocity3d(velocity)};

  // CAN WE SHORTCUT THE ABSTRACTION WITH A VECTOR OF QUANTITIES [UNIQUE] AS A QUANTITY VECTOR?
  ...
  // auto v0 = get<0>(velocity3d(velocity));
