...
<aside class="notes">
There are a couple of useful member types to see in addition to the storage matrix type, the row indexes tuple, and the column indexes tuple member types not shown here.
The storage underlying type, for example the float or double from the Eigen matrix.
The strong type of the i-th, j-th element, for example the velocity in meter per seconds from the quantity library.
Both of these aliases will also help the implementation.
</aside>
...
These members are static inline constexpr. We will omit all attribtutes in the slideware today for readability. The code is also slightly simplified at times, see the library on GitHub for the details.  
We will not deal with dynamically sized matrices today.  
</aside>
