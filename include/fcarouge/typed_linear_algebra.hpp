...
//! one reached only via a user-defined conversion or a shared base. `at<Type>()`
//! performs the exact, per-request uniqueness check for those residual cases.
//! It is not the negation of `uniform_typed_matrix`; the two are independent. A
