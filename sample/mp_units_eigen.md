...
^^ EXAMPLE FROM THE CPP GOES HERE ^^
...
Type safety cannot be guaranteed at compilation time without index-type safety. The indexes can either be non-type template parameters or strong types overloadings. Converting a runtime index to a dependent template type is not possible in C++. A proxy reference could be used to allow traditional assignment syntax but the runtime check and extra indirection are not interesting tradeoffs. A template call operator can be used for getting a type safe value but impractical syntax for setting. Without index safety, the accepted tradeoff is a templated index `at<i, j>()` method over the unchecked `operator[i, j]` and `operator(i, j)` accessors. Daniel Withopf [[6](https://www.youtube.com/watch?v=4LmMwhM8ODI)] uses strongly typed indexed to provide index safety. Other types of safeties should be considered such as reference frames and coordinate systems or taxonomy of the matrix.

The performance and zero-cost absraction should be measured and guaranteed.

The integration with `std::linalg` should be exhaustive.

The integration with compatible standard library support such as algorithms, ranges, mdspan explored and implemented.

Generalizing, elevating the dimension of the matrix, beyond the two present in common linear algebra.
