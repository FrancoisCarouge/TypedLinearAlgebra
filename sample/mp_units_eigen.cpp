...
[[maybe_unused]] auto sample{[] {
  ...
  using mp_units::si::unit_symbols::s4;
  ...
  //! @todo Mp-units seema to have support for vector quantity? Of the form:
  ...
  //! @todo Further compatibility with STL/Ranges/stdlinalg: sum_v=accumulate(
  ...
  using estimate_uncertainty = matrix<std::tuple<position, velocity, acceleration>, std::tuple<position, velocity, acceleration>>;
  using process_uncertainty = estimate_uncertainty;
  using output_uncertainty = quantity<m2>;
  using output_model = decltype(std::declval<output>() / std::declval<state>());
  using state_transition = decltype(std::declval<state>() / std::declval<state>());

  state x{0. * m, 0. * m / s, 0. * m / s2};
  estimate_uncertainty p{{500. * m2, 0. * m2 /s, 0. * m2 / s2},
                         {0. * m2 / s, 500. * m2 / s2, 0. * m2 / s3},
                         {0. * m2 / s2, 0. * m2 / s3, 500. * m2 / s4}};
  process_uncertainty q{ {0.25 * m2, 0.5 * m2 /s, 0.5 * m2 / s2},
                         {0.5 * m2 / s, 1. * m2 / s2, 1. * m2 / s3},
                         {0.5 * m2 / s2, 1. * m2 / s3, 1. * m2 / s4}};
  output_uncertainty r{9. * m2};
  output_model h{1. ,0. ,0.};
  state_transition f{{0., 1. * s, 0.5 * s2},
                     {0. * s, 0., 0. * s},
                     {0., 0., 0.}};
