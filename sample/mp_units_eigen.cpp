...
[[maybe_unused]] auto sample{[] {
  ...
  assert(s1[] == 1. * A / mol);
  /*
      // The singleton element can be accessed by conversion to its element
     type. using e = decltype(s1)::element<0, 0>; assert(e{s1} == 1. * A / mol);
      ...
      assert(std::format("{}", (x5 * xt5) * 2.) == "[[18 m², 12 m²/s, 6 m²/s²],"
                                                   " [12 m²/s, 8 m²/s², 4
     m²/s³]," " [6 m²/s², 4 m²/s³, 2 m²/s⁴]]"); assert(std::format("{}", 2. *
     (x5 * xt5)) == "[[18 m², 12 m²/s, 6 m²/s²]," " [12 m²/s, 8 m²/s², 4
     m²/s³]," " [6 m²/s², 4 m²/s³, 2 m²/s⁴]]");

      // More forms of multiplication with a strongly typed factor.
      assert(std::format("{}", x5 * (2. * m)) == "[[6 m²], [4 m²/s], [2
     m²/s²]]"); assert(std::format("{}", (2. * m) * x5) == "[[6 m²], [4 m²/s],
     [2 m²/s²]]"); assert(std::format("{}", xt5 * (2. * m)) == "[6 m², 4 m²/s, 2
     m²/s²]"); assert(std::format("{}", (2. * m) * xt5) == "[6 m², 4 m²/s, 2
     m²/s²]");
      ...
      assert(std::format("{}", x5 / (2. * m)) == "[[1.5], [1 1/s], [0.5
     1/s²]]"); assert(std::format("{}", (1. * m) / x5) == "[0.3333333333333333,
     0 s, 0 s²]"); assert(std::format("{}", xt5 / (2. * m)) == "[1.5, 1 1/s, 0.5
      1/s²]");
      ...
      // Beware of non-evaluated template expression: these types are not the
      same. auto a0{vector3d{1., 2., 3.} * mp_units::isq::velocity[m / s]};
      ...
      // Subtraction where both arguments should be of the same quantity kind
     and
      // character.
      //! @todo Add additional valid forms for other matrices.
      ...
      using position_2d_uncertainty =
          matrix<std::tuple<position, position>, std::tuple<position,
     position>>; position_2d_uncertainty p0;
      ...
      //! @todo Modulo where both arguments should be of the same quantity kind
      and
      //! character.
      ...
      //! @todo Further compatibility with STL/Ranges/std::linalg:
      sum_v=accumulate(
      //! v.cbegin()+1,v.cend()-1,0.0)
      //! @todo
      prod_v=accumulate(v.cbegin(),v.cend(),1.0,std::multiplies<float>())
      ...
      using state_transition =
          matrix<std::tuple<position, velocity, acceleration>,
                 std::tuple<quantity<one / m>, quantity<s / m>, quantity<s2 /
      m>>>; state_transition f; f.at<0, 0>() = 1.; f.at<0, 1>() = 1. * s;
     f.at<0, 2>() = 0.5 * s2; f.at<1, 1>() = 1.; f.at<1, 2>() = 1. * s; f.at<2,
     2>() = 1.; std::println("F: {}", f);
      // F: [[1, 1 s, 0.5 s²],
      //     [0 1/s, 1, 1 s],
      //     [0 1/s², 0 1/s, 1]]
      ...
      using gain =
          matrix<unevaluated_gain::row_indexes,
     unevaluated_gain::column_indexes>; gain k{p * transposed(h) / si};
