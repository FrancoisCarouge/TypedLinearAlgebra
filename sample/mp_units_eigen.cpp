...
[[maybe_unused]] auto sample{[] {
  ...
  //! @todo mp-units seem to have support for vectoro quantity? Of the form:
  ...
  // Tensor product of two vectors or tensors: a ⊗ b.

  // Inner product of two tensors: a ⋅ b.
  
  // Inner product of tensor and vector: a ⋅ b.
  
  // Scalar product of two tensors: a : b.

  //! @todo Further compatibility with STL/Ranges/stdlinalg.
  //sum_v=accumulate(v.cbegin()+1,v.cend()-1,0.0);

  //prod_v=accumulate(v.cbegin(),v.cend(),1.0,std::multiplies<float>());

  //is_all_zero=std::ranges::all_of(v,[](floatf){returnf==0;});

  //count_v=std::ranges::count_if(v,[](floatf){returnf>5.0;});

  //max_v=*std::ranges::max_element(v);

  //std::ranges::fill(v,5.0);

  //std::ranges::copy(v,w.begin());

  //std::ranges::copy_if(v,w.begin(),[](floatf){returnf>5.0;});

  //std::ranges::transform(v,w.begin(),[](floatf){returnf+1.0;});

  //x=inner_product(v.cbegin(),v.cend(),w.begin(),0.0);
