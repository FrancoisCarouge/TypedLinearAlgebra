...
//! @file
//! @brief Unit safe linear algebra with mp-units and Armadillo.
//!
//! @details Demonstrate a variety of linear algebra operations with mp-units
//! and Armadillo. This library composes Armadillo as the matrix' linear algebra
//! backend with index typed as mp-units types. This sample explicitly uses
//! double precision floating point numbers. This sample uses Armadillo linear
//! algebra as the linear algebra backend. This sample uses mp-units types for
...
[[maybe_unused]] const auto sample{[] -> int {
  ...
  // Singleton division reduces to a scalar quotient.
  assert(std::format("{}", s1 / s1) == "1");

  //! @note Vector and matrix division (`1. / x5`, `x5 / x5`, ...) resolves to
  //! a least-squares solve, which needs LAPACK. This facade fetches Armadillo
  //! in pure header-only mode, without LAPACK, so those forms are unavailable
  //! with the Armadillo backend. See the Eigen sample for that demonstration.
