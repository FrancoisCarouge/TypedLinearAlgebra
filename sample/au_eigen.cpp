...
[[maybe_unused]] const auto sample{[] -> int {
  ...
  // producing an area typed matrix (unlike mp-units, an unqualified `matrix *
  // bare quantity` expression is ambiguous with Au's own hidden friend
  // `operator*` found via argument-dependent lookup on the quantity operand,
  // so this sample sticks to matrix-by-matrix and matrix-by-plain-scalar
  // forms).
